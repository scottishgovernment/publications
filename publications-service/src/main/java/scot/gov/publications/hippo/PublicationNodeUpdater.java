...
public class PublicationNodeUpdater {
    ...
    public Node createOrUpdatePublicationNode(Metadata metadata, Publication publication)
        ...
        try {
            ...
            topicMappings.addTopicsIfAbsent(publicationNode, metadata);
     ...
     * it will not be overwritten with this data.  We may want to revise thsi later as we do not want manual edits
     * to be needed.
     */
    private void createDirectoratesIfAbsent(Node publicationNode, Metadata metadata) throws ApsZipImporterException, RepositoryException {
        ...
        if (!publicationNode.hasNode("govscot:primaryResponsibleDirectorate") &&
                isNotBlank(metadata.getPrimaryResponsibeDirectorate())) {
            createDirectorateLink(
                    publicationNode,
                    "govscot:primaryResponsibleDirectorate",
                    metadata.getPrimaryResponsibeDirectorate());
        }
    ...
    private void createRolesIfAbsent(Node publicationNode, Metadata metadata)
        ...
        if (!publicationNode.hasNode("govscot:primaryResponsibleRole") && isNotBlank(metadata.getPrimaryResponsibeRole())) {
            createRoleLink(publicationNode, "govscot:primaryResponsibleRole", metadata.getPrimaryResponsibeRole());
        }
